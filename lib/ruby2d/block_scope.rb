  ...
  module BlockScope
    ...
    def scoping *names
      ...
      names.each do |s|
        ...
        alias_method os, s
        masking_active! mab
        define_method s do |*a, scope_over: true, **na, &b|
          r = self.send(os, *a, **na)
          if b
            return scope r, scope_over: scope_over do
              b.call r
            end
          end
          return r
    ...
    class ScopeValet
      ...
      def scope(mask, scope_over: true)
        _push mask
        r = yield mask
        _pop
        if scope_over
          if scope_over == true
            return mask.scope_over r if mask.respond_to? :scope_over
          else
            return mask.send scope_over
          end
        end
        return mask
      end
    ...
    module Includes
      ...
      def scope(mask, scope_over: true, &b)
        if @scope_valet
          @scope_valet.scope(mask, scope_over:, &b)
        else
