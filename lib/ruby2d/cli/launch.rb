...
# Launch a web app
def launch_web
  unless File.exist? "build/app.html"
...
def launch_apple(device)
  case device
  when "ios"
    unless File.exist? "build/ios/build/Release-iphonesimulator/MyApp.app"
      puts "No iOS app built!"
      exit
    end
    puts %x(simple2d simulator --open "iPhone X" &&
          simple2d simulator --install "build/ios/build/Release-iphonesimulator/MyApp.app" &&
          simple2d simulator --launch "Ruby2D.MyApp")
  when "tvos"
    unless File.exist? "build/tvos/build/Release-appletvsimulator/MyApp.app"
      puts "No tvOS app built!"
      exit
    end
    puts %x(simple2d simulator --open "Apple TV 4K" &&
