module Ruby2D
    class FitGrid < Grid
        ...
        def arrange(object, col, row, *a)
            col = col..col if not col.is_a? Range
            row = row..row if not row.is_a? Range
            ...
            a << :x if not a.any_in? :x, :left, :right
            a << :y if not a.any_in? :y, :top, :bottom
        ...
        def fit(dir, dim)
            ...
            @object_sectors.sort{|a, b| a.send(dir).max <=> b.send(dir).max}.each do |os|
                x[os.send(dir).max] = [x[os.send(dir).max], os.object.send(dim).get - x[os.send(dir).min...os.send(dir).max].sum].max
            end
            @object_sectors.sort{|a, b| a.send(dir).min <=> b.send(dir).min}.each do |os|
                n[os.send(dir).min] = [n[os.send(dir).min], os.object.send(dim).get - n[os.send(dir).min + 1..os.send(dir).max].sum].max
            end
...
#             col = col..col if not col.is_a? Range
#             row = row..row if not row.is_a? Range
#             @cols.set{_1 + Array.new(col.max - _1.length + 1, 0)} if @cols.get.length <= col.max
#             @rows.set{_1 + Array.new(row.max - _1.length + 1, 0)} if @rows.get.length <= row.max
#             sector = sector(col, row)
#             a << :x if not a.any_in? :x, :left, :right
#             a << :y if not a.any_in? :y, :top, :bottom
#             object.plan **a.map{[_1, sector.send(_1)]}.to_h
