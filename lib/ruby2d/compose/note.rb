module Ruby2D
    class Note < Widget
        ...
        def close_type_story
            story_push(Selection.new(@type_story[:start], @type_story[:length]), @type_story[:text], @type_story[:start_selection])
            @type_story = nil
        end
        ...
        def story_back
            close_type_story if @type_story
            if @story_index > 0 
                @story.push(PasteStoryEntry.new @selection.get, @text_value.get, Selection.new) if @story.size == @story_index
        ...
        def story_front
            if @story_index + 1 < @story.size && @story[@story_index + 1].front(@text_value, @selection, @pen_position, @story[@story_index])
                @story_index += 1
            end
        end
